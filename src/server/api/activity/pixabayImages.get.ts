...
export default defineEventHandler(async (event) => {
	...
	const url = `https://pixabay.com/api/?key=${config.pixabayApiKey}&q=${encodeURIComponent(query)}&safesearch=true&per_page=50&page=${page0}`;

	try {
		const response = await $fetch<{ hits: PixabayImage[]; total: number; totalHits: number }>(url, {
			headers: {
		...
		return {
			...
			page: page0,
			hasMore: page0 * 50 < response.totalHits
		};
	} catch (error) {
		console.warn('Pixabay images unavailable; returning empty result set.', error);
		return {
			hits: [],
			total: 0,
			totalHits: 0,
