...
export default defineEventHandler(async (event) => {
	...
	const { id, name, generate } = getQuery(event);
	...
	if (generate) {
		...
		if (generate !== 'true') {
			throw createError({
				statusCode: 400,
				statusMessage: 'Generate parameter, if provided, must be "true"'
			});
		}
		...
		if (!name || typeof name !== 'string' || name.trim() === '') {
			throw createError({
				statusCode: 400,
				statusMessage:
					'Name parameter is required and must be a non-empty string when generating thumbnail'
			});
		}

		const res = await $fetch(
			`${config.public.cloudBaseUrl}/v1/events/thumbnail/${id}/generate?name=${encodeURIComponent(name)}`,
			{
				method: 'POST',
				headers: {
