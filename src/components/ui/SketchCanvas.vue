		...
		<div class="mt-3! px-1! flex flex-col gap-2!">
			<div class="flex gap-1.5 flex-wrap items-center">
				<button
					v-for="c in palette"
					:key="c"
					class="rounded-full! border-[3px]! shrink-0 transition-transform duration-100"
					:class="props.compact ? 'w-6! h-6!' : 'w-7! h-7!'"
					:style="{
						background: c,
						borderColor:
			...
			<div class="flex flex-wrap items-center gap-1.5!">
				<button
					v-for="s in sizes"
					:key="s"
					class="flex items-center justify-center rounded-lg! border! shrink-0 transition-colors"
					:class="[
						props.compact ? 'w-7! h-7!' : 'w-8! h-8!',
						penSize === s ? 'border-white/70 bg-white/15' : 'border-neutral-700'
					]"
					@click="penSize = s"
				>
					<span
...
<script lang="ts">
// canvas persistence across mounts, keyed so multiple instances don't trample each other
const persistedCanvases = ref<Record<string, string>>({});
</script>
...
// compact mode keeps the 6 most distinct colors so tiny widgets stay readable
const COMPACT_PALETTE = ['#ef4444', '#eab308', '#22c55e', '#3b82f6', '#000000', '#ffffff'];
const FULL_SIZES = [4, 8, 16, 28];
const COMPACT_SIZES = [4, 8, 16];
const FULL_TOOLS = [
...
// compact mode drops fill since flood fill needs a more deliberate canvas
const COMPACT_TOOLS = [
	{ id: 'pen', icon: 'i-lucide-pen-line', label: 'Pen' },
	{ id: 'eraser', icon: 'i-lucide-eraser', label: 'Eraser' }
];
...
const palette = computed(() => (props.compact ? COMPACT_PALETTE : FULL_PALETTE));
const sizes = computed(() => (props.compact ? COMPACT_SIZES : FULL_SIZES));
const tools = computed(() => (props.compact ? COMPACT_TOOLS : FULL_TOOLS));

const tool = ref<'pen' | 'eraser' | 'fill' | 'line' | 'rect' | 'circle' | 'triangle' | 'octagon'>(
	'pen'
);
const currentColor = ref<string>(palette.value[0]!);
const customColor = ref('#ff0000');
const penSize = ref<number>(sizes.value[1] ?? sizes.value[0]!);
const opacity = ref(1);
const isDrawing = ref(false);
const canUndo = ref(false);
const isCustom = computed(() => !palette.value.includes(currentColor.value));
...
function getPos(e: PointerEvent) {
	...
	return {
		x: Math.round((e.clientX - rect.left) * (props.canvasSize / rect.width)),
		y: Math.round((e.clientY - rect.top) * (props.canvasSize / rect.height))
	};
}

function saveHistory() {
	history.push(ctx().getImageData(0, 0, props.canvasSize, props.canvasSize));
...
function selectColor(c: string) {
	...
	if (!palette.value.includes(c)) customColor.value = c;
...
function onDown(e: PointerEvent) {
	...
	if (isShapeTool.value) {
		...
		shapeSnapshot.value = ctx().getImageData(0, 0, props.canvasSize, props.canvasSize);
...
function persist() {
	...
	persistedCanvases.value[props.persistKey] = canvasEl.value.toDataURL('image/png');
}
...
function floodFill(startX: number, startY: number) {
	const c = ctx();
	const imageData = c.getImageData(0, 0, props.canvasSize, props.canvasSize);
	const { data } = imageData;
	const w = props.canvasSize;
...
onMounted(() => {
	...
	const stored = persistedCanvases.value[props.persistKey];
	if (stored) {
		const img = new Image();
